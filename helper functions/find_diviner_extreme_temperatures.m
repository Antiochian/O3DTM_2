function [Tmax_matrix, Tmin_matrix, Tmax_ltim_matrix, Tmin_ltim_matrix, Tmax_dtm_matrix, Tmin_dtm_matrix, lat_arr, long_arr, Tmean_matrix] = ...
    find_diviner_extreme_temperatures(ppd, required_lat_arr, required_long_arr, gen_mean_temperatures)
...
if gen_mean_temperatures
    Ttot_matrix = zeros(size_arr);
    Tcount_matrix = zeros(size_arr);
end
...
for file_idx = 1:length(tbol_jp2_names)
    ...
    if file_idx > 1 && (ppd == 128 || nargin == 4)
    ...
    if gen_mean_temperatures
        NaN_mask = isnan(tbol_matrix);
        count_matrix = ones(size_arr);
        count_matrix(NaN_mask) = 0;
        tbol_matrix(NaN_mask) = 0;
        Ttot_matrix = Ttot_matrix + tbol_matrix;
        Tcount_matrix = Tcount_matrix + count_matrix;
    end
...
if gen_mean_temperatures
    Tmean_matrix = Ttot_matrix./Tcount_matrix;
else
    Tmean_matrix = NaN;
end
%% Define functions
